...
namespace testeCampos
{
   public class Relacao
    {
        public void geraRelacao(List<string> filesAdionado, Excel.Application MyApp, string caminho, string directoryPath, string nomeSheet, string excelConnectionString, List<string> colunas, List<string> colunasCreate, List<String> itemsDataGrid, DataGridView dataGridView1, SqlConnection conn, int sheet)
        {
            foreach (string element in filesAdionado)
            {
                ...
                using (OleDbConnection connection = new OleDbConnection(excelConnectionString))
                {
                    ...
                    for (int a = 0; a < dataGridView1.Rows.Count; a++)
                    {
                        if (dataGridView1.Rows[a].Cells[1].Value.ToString() != "" && dataGridView1.Rows[a].Cells[1].Value.ToString() != "Rel_ID")
                        {
                    ...
                    for (int f = 0; f < itemsDataGridInsert.Count; f++)
                    {
                        if (f < itemsDataGrid.Count - 1)
                        {
                            if (itemsDataGridInsert[f] == "Rel_PA_Pro_ID")
                            {
                                camposTabelaInsert.Append(" [" +Convert.ToString(itemsDataGrid[f]).Replace('.', '#') +" ], " );
                            }
                            else if (itemsDataGridInsert[f] == "Rel_MP_Pro_ID")
                            {
                                 camposTabelaInsert.Append(" [" +Convert.ToString(itemsDataGrid[f]).Replace('.', '#') +" ], " );
                            }
                            else if(itemsDataGridInsert[f] == "Rel_Produzida")
                            {
                                camposTabelaInsert.Append(" (convert(decimal(12,4), replace([" + Convert.ToString(itemsDataGrid[f]).Replace(".", "#") + "], ',' , '.'))), ");
                            }
                            else if (itemsDataGridInsert[f] == "Rel_Relacao")
                            {
                                camposTabelaInsert.Append(" (convert(decimal(12,4), replace([" + Convert.ToString(itemsDataGrid[f]).Replace(".", "#") + "], ',' , '.'))), ");
                            }
                            else if (itemsDataGridInsert[f] == "Rel_Tipo")
                            {
                                camposTabelaInsert.Append(" [" + Convert.ToString(itemsDataGrid[f]).Replace('.', '#') + " ], ");
                            }
                            else
                        ...
                        else
                        {
                            ...
                            {
                                camposTabelaInsert.Append(" (convert(decimal(12,4), replace([" + Convert.ToString(itemsDataGrid[f]).Replace(".", "#") + "], ',' , '.'))), ID");
                            }
                            else if (itemsDataGridInsert[f] == "Rel_Relacao")
                            {
                                camposTabelaInsert.Append(" (convert(decimal(12,4), replace([" + Convert.ToString(itemsDataGrid[f]).Replace(".", "#") + "], ',' , '.'))), ID");
                   ...
                   " INSERT INTO [dbo].[D_Relacao_Carga] ( " + camposTabelaSQL + " ) " +
                   " SELECT " + camposTabelaInsert +
                   " FROM [dbo].[Relacao_Carga] ";
                     //  " WHERE " + camposWhere;
                   ...
                   MessageBox.Show(cmdCopPedido.CommandText);
                    ...
                    try
                    {
                        ...
                        tr.Commit();
                        MessageBox.Show("Tabela Relacao de Produção Copiada ");
                    }
                    catch (Exception ex)
                    {
                        tr.Rollback();
